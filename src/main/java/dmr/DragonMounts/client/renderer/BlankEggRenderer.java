    ...
    public static class MultiplyAlphaRenderTypeBuffer implements MultiBufferSource {
        ...
        public VertexConsumer getBuffer(RenderType type) {
            ...
            if (localType instanceof RenderType.CompositeRenderType) {
                ...
                ResourceLocation texture = ((RenderStateShard.TextureStateShard)
                                ((RenderType.CompositeRenderType) localType).state.textureState)
                        .texture.orElse(InventoryMenu.BLOCK_ATLAS);

                localType = entityTranslucentCull(texture);
            } else if (localType
                    .toString()
                    .equals(Sheets.translucentCullBlockSheet().toString())) {
